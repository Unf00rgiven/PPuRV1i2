...
int main(int argc, char* argv[])
{
	...
	uint_least8_t* outData = (uint_least8_t*) malloc(3 * bitmapInfoHeader.biWidth * bitmapInfoHeader.biHeight * sizeof(uint_least8_t));

	averageRGB(bitmapData, bitmapInfoHeader.biWidth, bitmapInfoHeader.biHeight);
	...
	meanFilter(bitmapData, bitmapInfoHeader.biWidth, bitmapInfoHeader.biHeight, outData);
	err = SaveBitmapFile("out_meanFilter.bmp", &bitmapFileHeader, &bitmapInfoHeader, outData);
	if (err)
	{
		printf("Error trying to save bitmap\n");
	}
